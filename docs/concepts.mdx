...
internal but only external. We call these pipelines **root pipelines**.
...
sub-pipeline is started, it either _shares_ or _clones_ its parent's context, depending on
which joint filter you use. For instance, a _link_ filter shares its parent's context
while a _demux_ filter clones it.

To the scripts embedded in a pipeline, these **context variables** are their **global varaibles**,
...
proxy needs to handle a large amount of connections in parallel. Each should have its own state
