...
If `condition` is true, `value1` is returned. Otherwise, `value2` is returnd.
...
Global variables in PipyJS are also called "_context variables_". Refer to [Context](/concepts#context) for a deep dive into the concepts of _context_ and _context variables_.
...
Global variables are scoped within a single file or [_module_](/concepts#module), and can be shared between different modules by using [export()](/reference/api/Configuration/export) and [import()](/reference/api/Configuration/import).
