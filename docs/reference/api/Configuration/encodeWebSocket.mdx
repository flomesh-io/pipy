...
An **encodeWebSocket** filter encodes its input [Messages](/reference/api/Message) into [WebSocket](https://en.wikipedia.org/wiki/WebSocket) messages and outputs encoded [Data](/reference/api/Data). Its input is a _Message_ stream and its output is a _Data_ stream.
