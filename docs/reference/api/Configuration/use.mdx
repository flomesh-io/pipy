...
A **use** filter and its sub-pipelines's input and output can be any events.
...
A **use** filter pumps events through a sub-pipeline in a different module, or a series of sub-pipelines in a module chain. You can use it in multiple ways as being discussed down below.
