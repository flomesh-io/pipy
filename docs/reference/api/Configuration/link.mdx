...
A **link** filter and its sub-pipeline's input and output can be any events.
...
A **link** filter creates a new sub-pipeline and pumps all events through it, as if the new sub-pipeline has taken the place of the link filter.
