...
A **fork** filter and its sub-pipelines's input and output can be any events.
...
A **fork** filter creates one or more sub-pipelines, each receiving a clone of all events from the filter's input. The fork filter's output is exactly the same as its input. All output from the created sub-pipelines are discarded.
