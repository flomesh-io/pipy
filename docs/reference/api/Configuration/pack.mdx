...
You can also let the filter output a combined _Message_ after a certain amount of time even when the input _Messages_ haven't reached to _batchSize_ yet. This is done by the _timeout_ option in the _options_ parameter, which can be a number in seconds, or a string with a time unit suffix such as `'s'`, `'m'`, `'h'` or `'d'`.
