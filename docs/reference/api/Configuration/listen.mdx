...
Set to _false_ to enable _half-open_ of a TCP connection after receiving _EOF_ from the client. You would also set the _readTimeout_ option as well or the connection could be lingering forever if no _StreamEnd_ events coming out of the pipeline. Default is _true_.
