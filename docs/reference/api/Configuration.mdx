...
Use [listen()](/reference/api/Configuration/listen) to create a pipeline layout that generates a root pipeline for every incoming connection on a specified TCP port. Use [task()](/reference/api/Configuration/task) to create a pipeline layout that generates root pipelines regularly for periodic tasks. Use [pipeline()](/reference/api/Configuration/pipeline) to create a pipeline layout for sub-pipelines.
