...
Use [`export()`](/reference/api/Configuration/export) to define global variables that are visible to other modules. Use [`import()`](/reference/api/Configuration/export) to import global variables defined and exported from other modules.

### Methods for defining pipelines

Use [`listen()`](/reference/api/Configuration/listen) to create a root pipeline that listens on a specified TCP port. Use [`task()`](/reference/api/Configuration/task) to create a root pipeline that gets a message regularly for periodic tasks. Use [`pipeline()`](/reference/api/Configuration/pipeline) to create a sub-pipeline by a given name.
...
* [`replaceData()`](/reference/api/Configuration/handleData)
* [`replaceMessage()`](/reference/api/Configuration/handleMessage)
* [`replaceMessageBody()`](/reference/api/Configuration/handleMessageBody)
* [`replaceStreamEnd()`](/reference/api/Configuration/handleStreamEnd)
* [`replaceStreamStart()`](/reference/api/Configuration/handleStreamStart)
