...
covering other common proxy features. The first one is "_load balancing_".
...
In this tutorial, we'll be using [_algo.RoundRobinLoadBalancer_](/reference/classes/algo/RoundRobinLoadBalancer),
...
[_select()_](/reference/classes/algo/RoundRobinLoadBalancer/select) method, it will
...
> [_Array.prototype.map_](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array/map).
> For objects though, we have to convert them first to an array of `[key, value]`s, transform it by `map()`,
...
Since we already have service-to-target mapping in the balancer, here we change
