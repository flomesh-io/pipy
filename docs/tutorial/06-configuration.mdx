...
> We use [_JSON.decode()_](/reference/classes/JSON/decode) instead of the standard
...
2. Use [_pipy.load()_](/reference/functions/pipy.load) to read files as binary data.
3. Use [_JSON.decode()_](/reference/classes/JSON/decode) to decode JSON from binary data.
