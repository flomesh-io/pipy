...
> We use [_JSON.decode()_](/reference/api/JSON/decode) instead of the standard
...
2. Use [_pipy.load()_](/reference/api/pipy.load) to read files as binary data.
3. Use [_JSON.decode()_](/reference/api/JSON/decode) to decode JSON from binary data.
