...
可以用来做限流的过滤器有两个 [`throttleMessageRate`](/reference/Configuration/throttleMessageRate) 和 [`throttleDataRate`](/reference/Configuration/throttleDataRate)，前者限制每秒的最大请求数，后者限制每秒最大的字节数。二者的参数一样：
