...
2. 点击上面工具栏中的 **+** 按钮来创建新文件。弹出窗口中输入 `/hello.js`并点击 **Create**。
3. 在左侧文件列表中选中刚创建的文件，然后点击上面工具栏中的旗帜图标，将该文件设置为程序的入口脚本。
...
1. 第 1 行中调用内置的函数 [_pipy()_](/reference/pipy) 将返回一个 [`Configuration`](/reference/Configuration) 对象。后面我们使用这个对象来定义 *管道（pipeline）* 和 *过滤器（filter）*。
...
3. 在第 4 行，我们为了 2 中创建的管道添加 [`serveHTTP`](/reference/classes/Configuration/serveHTTP) 过滤器。过滤器使用 [`Message`](/reference/classes/Message) 作为构造参数，并在响应中输出同样类型的消息。
...
* Pipy 程序由一系列的*管道*和*过滤器*的定义组成。从 [_pipy()_](/reference/pipy) 的调用开始，通过调用其返回 [`Configuration`](/reference/Configuration) 对象的多个方法来定义管道和过滤器。
