...
2. 在第 3 行中我们通过调用前面创建的 `Configuration` 对象的 [_listen()_](/reference/api/Configuration/listen) 方法，定义一个监听在 `8080` 端口的*端口管道（port pipeline）*。这个函数会返回该 `Configuration` 对象，因此后面可以继续调用它的其他方法来定义更多的*管道*和*过滤器*。
