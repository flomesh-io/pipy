...
Pipy provides class [_URLRouter_](/reference/classes/algo/URLRouter) for
...
the parameter given to [_pipy()_](/reference/functions/pipy) at the beginning of
...
[_URLRouter.find()_](/reference/classes/algo/URLRouter/find) to work out the value of
...
(See [Context](/concepts#context) in _Concepts_ for more about _global variables_).
...
alive and has inputs. At that point, `_target` must exist so you can use it safely.
...
At definition, [_link()_](/reference/classes/Configuration/link) accepts one or more
...
For more about pipeilnes and filters, see [Filter and pipeline](/concepts/#filter-and-pipeline)
in _Concepts_.
...
1. Use [_algo.URLRouter_](/reference/classes/algo/URLRouter) to map a URL-like string
