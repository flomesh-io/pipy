...
To solve this problem, we change the strategy how **muxHTTP()** instances share sub-pipelines by giving it the variable `_target` for the _group sharing_ criterion, so that sub-pipelines will only be shared by requests targeting to the same server, regardless of what client connection.
