...
The opposite to **demuxHTTP()** is **muxHTTP()**. Unlike **demuxHTTP()** spawning multiple sub-pipelines, **muxHTTP()** _shares_ a sub-pipeline with other **muxHTTP()** filters.

The filter accepts an optional parameter for deciding what other **muxHTTP()** filters it shares sub-pipeline with. When two **muxHTTP()** filters' parameters have the same value (or return the same value if they're functions), they'll share the same sub-pipeline.

By default, the parameter is `__inbound`, which is a built-in _context variable_ representing the current downstream connection. So by default, all **muxHTTP()** filters working for the same downstream connection will share one single sub-pipeline.
