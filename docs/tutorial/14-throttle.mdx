...
Pipy built-in filters [`throttleMessageRate`](/reference/Configuration/throttleMessageRate) and [`throttleDataRate`](/reference/Configuration/throttleDataRate) are used to implement rate limiting, where former is used for maximum number of requests per second and latter is used for maximum number of bytes per second. Both accept a similar set of parameters:
