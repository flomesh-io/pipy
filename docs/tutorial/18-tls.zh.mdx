...
[`acceptTLS`](/reference/Configuration/acceptTLS) 过滤器用于接受 TLS 加密的数据流，并进行 TLS 卸载。其接受两个参数 `acceptTLS(target, options)`：
...
在过滤器 `acceptTLS` 中除了指定子管道以外，还通过 `options` 选项在指定 TLS 要使用的证书和秘钥（*certificate*）。[`CertificateChain`](/reference/crypto/CertificateChain) 用于创建证书对象，[`PrivateKey`](/reference/crypto/PrivateKey) 用于创建私钥。
