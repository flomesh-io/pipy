...
Pipy 提供各种内置类让实现负载均衡代理更容易。这些类实现了各种的负载均衡算法，提供了同样的使用方式。在本节中我们将使用的 [algo.RoundRobinLoadBalancer](/reference/algo/RoundRobinLoadBalancer) 实现了常见了“轮询（round-robin）”算法。
...
有了 *RoundRobinLoadBalancer*，每次调用其 [select()](/reference/algo/RoundRobinLoadBalancer/select) 方法都会以轮询的方式返回其中的一个目标。
...
第二部分其实可以做在 “*均衡器*” 插件中，而第一部分继续放在“*路由*”插件中。除此以外 “路由器” 还应通知 “均衡器” 插件要使用的服务。前面我们已见到 [05 插件](/tutorial/05-plugins#export-and-import) 中在 `/proxy.js` 和 `plugins/router.js` 之间共享的变量 `_turnDown`。这里也使用同样的方式。
...
这次我们学习了如果写一个轮训的负载均衡器，同时学会了只用一行代码转换 JSON 对象的技巧。

### 收益

1. 使用 [_algo.RoundRobinLoadBalancer_](/reference/algo/RoundRobinLoadBalancer)（还有其他类型的均衡器）自动从一组目标服务器中选择，并结合各自的权重。
