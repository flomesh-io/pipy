...
[_RoundRobinLoadBalancer.prototype.select()_](/reference/classes/algo/RoundRobinLoadBalancer/select)
...
We make such a cache by using [_algo.Cache_](/reference/classes/Cache),
...
> Call to [_RoundRobinLoadBalancer.prototype.deselect()_](/reference/classes/algo/RoundRobinLoadBalancer/deselect)
> is not a must, but in case of some other types of load balancing algorithms
> like [_LeastWorkLoadBalancer_](/reference/classes/algo/LeastWorkLoadBalancer),
...
we call [_algo.Cache.prototype.get()_](/reference/classes/algo/Cache/get)
...
Lastly, when the incoming connection closes, we clear up the cache.
...
1. Use [_algo.Cache_](/reference/classes/Cache) to remember what targets
