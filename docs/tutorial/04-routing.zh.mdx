...
Pipy 提供了类 [URLRouter](/reference/api/algo/URLRouter)，用于快速将 URL 映射成任何类型的值。在这个场景中，我们应该将其映射成包含目标地址和端口的*字符值*。
...
在[之前的教程中](/tutorial/02-echo/#代码剖析)我们已经认识了*内置*全局变量 `__inbound`。这次我们定义一个名为 `_router` 的*自定义全局变量*。通过在 [_pipy()_](/reference/api/pipy) 函数的参数来实现。重新修改下上次的代码：
...
此时我们已经有了所有需要的变量，下一步就是调用  [*URLRouter.find()*](/reference/api/algo/URLRouter/find) 根据请求计算出 `_target` 的值。我们可以 *demuxHTTP* 和 *muxHTTP* 之间使用 *handleMessageStart* 过滤器中完成这一步。由于 *demuxHTTP* 和 *muxHTTP* 位于不同的管道中，因此需要在其二者间插入一个新的子管道来连接他们。新的子管道名为 “*request*”。
...
这是因为自定义全局变量总是绑定到某个管道上。因此，其只存在于工作的管道中（参考概念中的 [上下文](/intro/concepts#上下文) 了解更多*全局变量*的信息）。假如没有入站的连接，就没有管道，也就不会有这些变量存在。
...
使用时，[link()](/reference/api/Configuration/link) 可以接受一对或多对参数：每对参数中第一项是目标管道的名字，紧跟着一个可以决定是否选择该管道的条件回调函数。只有一对参数可以省略回调函数，表示为默认选项。
...
更多管道和过滤器相关的内容，可以参考*概念*中的[过滤器和管道](/intro/concepts/#过滤器和管道)。
...
1. 根据路由表，使用 [algo.URLRouter](/reference/api/algo/URLRouter) 将类 URL 的字符串映射成任意类型的值，基于要实现的路由代理。
