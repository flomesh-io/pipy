...
这是因为自定义全局变量总是绑定到某个管道上。因此，其只存在于工作的管道中（参考概念中的 [上下文](/intro/concepts#context) 了解更多*全局变量*的信息）。假如没有入站的连接，就没有管道，也就不会有这些变量存在。
...
更多管道和过滤器相关的内容，可以参考*概念*中的[过滤器和管道](/intro/concepts/#filter-and-pipeline)。
