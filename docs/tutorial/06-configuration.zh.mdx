...
> 这里我们并没有使用标准的 [JSON.parse()](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse)，而是使用 [JSON.decode](/reference/JSON/decode)，因为前者只能处理字符串而后者可以处理二进制数据。
...
这次我们还是没有增加任何功能。可以继续用[上次的方式](/tutorial/05-plugins#test-in-action)进行测试。

## 总结

在这次教程中，我们介绍了了如何将逻辑代码与配置数据分离，可以更优雅地组织那些有很多配置项的代码。

### 收益

1. 代码组织的最佳实践之一就是代码与数据的分离和提供代码的可配置性。
2. 使用 [`pipy.load()`](/reference/pipy.load) 从文件中读取二进制数据。
3. 使用 [`JSON.decode()`](/reference/JSON/decode) 从二进制数据中解码 JSON 对象。
