...
Autentication is a critical part of services and almost each service implements some kind of authentication mechanism. A complex system usually implement or uses single sign-on (SSO) or something similar to avoid users of its systems going through authentication process repeatedly. Generally there are two kind of authentication options:
...
We will give you a very simple brief of JWT, to get better understanding of what JWT is, its purpose and benefits it bring, please refer to official site for [JWT Introduction](https://jwt.io/introduction).
...
Pipy's [crypto](/reference/crypto) package contains security related classes。We will be using few of them in this tutorial like [`PrivateKey`](/reference/crypto/PrivateKey) and [`JWT`](/reference/crypto/JWT). The former is related to handle private key, while the latter is used for JWT verification.
