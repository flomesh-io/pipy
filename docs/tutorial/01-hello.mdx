...
1. At the very first line, the builtin global function [_pipy()_](/reference/pipy)
   is called. It returns a [_Configuration_](/reference/Configuration)
...
2. At line 3, we define a _port pipeline_ that listens on port 8080 by calling method
   [_listen_()](/reference/Configuration/listen) of the _Configuration_ object
...
* A Pipy program is made of definitions for _pipelines_ and _filters_. It always
  starts off with a call to [_pipy()_](/reference/pipy), which gives us
  a [Configuration](/reference/Configuration) object, which we call various
