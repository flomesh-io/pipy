...
As you can see from above, Pipy is fundamentally an _"expression evaluator"_. If you say _"pipy -e 1+1"_, it tells you _"2"_,  nothing fancy indeed. Surely that isn't what we created Pipy for. To make use of Pipy in a more meaningful way, we should write an expression that evaluates to a _Configuration_.
...
3. At line 3, we add a [serveHTTP()](/reference/classes/Configuration/serveHTTP) filter 
   to the pipeline layout that we just added in step 2. The filter takes a [Message](/reference/classes/Message)
...
* A _port pipeline_ reads from a network port and writes back whatever its last filter outputs.
