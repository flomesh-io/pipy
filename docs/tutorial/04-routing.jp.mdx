...
PipyはURLからあらゆるタイプの値への高速なマッピングクラス、 [_URLRouter_](/reference/algo/URLRouter) を提供します。ここでは、ターゲットのアドレスとポートを持つ文字列の値にマップしたいと思います。
...
[パート2](/tutorial/02-echo/#code-dissection-1)では、ビルトインのグローバル変数`__inbound`を扱いました。今回は`_router`というカスタムグローバル変数を定義します。これは、スクリプトの冒頭で [_pipy()_](/reference/pipy) に与えたパラメーターを通じて行います。そこで先に行ったものに再び追加します。
...
以上で、必要な変数はできました。次は[_URLRouter.find()_](/reference/algo/URLRouter/find) をコールして、リクエストの内容から、`_target` の値を求めます。これは、_demuxHTTP_ と _muxHTTP_ の間の _handleMessageStart_ フィルターで行います。 _demuxHTTP_ と _muxHTTP_ は同じパイプラインにないので、新しいサブパイプラインをこの間に挿入します。新しいサブパイプラインの名前は _request_ です。
...
これは、カスタムグローバル変数が常に何らかのパイプラインに付随しているからです。つまり、パイプラインがライブになっている時だけ、これらの変数もライブになります（グローバル変数の詳細は _Concepts_ の [Context](/concepts#context) を参照してください）。もし、着信接続がなければパイプラインもないので、これらの変数もまだ存在しないことになります。
...
[_link()_](/reference/Configuration/link) がコールされると、1組あるいは複数組のペアのパラメーターを受け取ります。それぞれのペアでは、ターゲットとなるパイプラインの名前が最初に来て、その後に、このサブパイプラインを選択すべきかどうかを決定するコールバック関数が続きます。最後のペアは、それが最終のデフォルトを選択する場合には、そのコールバック関数を省略できます。
...
パイプラインとフィルターについて詳しくは、 _Concept_ の [Filter and pipeline](/concepts/#filter-and-pipeline) を参照してくだざい。
...
1.	[_algo.URLRouter_](/reference/algo/URLRouter)を使用して、URLのような文字列をルーティングテーブルに従ってあらゆる種類の値にマップします。これに従えば、単純なルーティングプロキシを実装できます。
