...
from the plugins. We do this with [_use()_](/reference/Configuration/use),
...
We define it with [_export()_](/reference/Configuration/export) in `proxy.js`
...
1. Use [_use()_](/reference/Configuration/use) to link to pipelines
   defined in other files and form a _plugin chain_. It is the key to
   an extendable plugin architecture.

2. Global variables defined with [_pipy()_](/reference/pipy)
   are visible only within its definition file. To shared global variables
   across different files, use [_export()_](/reference/Configuration/export)
   and [_import()_](/reference/Configuration/import).
