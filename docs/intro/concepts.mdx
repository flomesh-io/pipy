...
Pipy is a proxy that works like a "_stream processor_", which means that it takes in _streams_, processes _streams_, and spits out _streams_.
...
Unlike what people usually think _streams_ are, Pipy streams are made of **events** instead of bytes. There are four types of events:
...
Eventually, all events left in the stream after the processing are sent back to network. At this point, all other events except **Data** are discared.
...
A **context** is a set of variables attached to a pipeline, used by scripts to maintain the current state of a pipeline.
