...
Pipy 是一个代理，它就像“*流处理器*”一样接收*流*、处理然后将其作为*流*输出。
...
* 在入站一侧，输入流是从客户端流向 Pipy，而输出流则是从 Pipy 发回客户端。
* 在出站一侧，输出流是从 Pipy 发往服务器，而输入流是从服务器发回 Pipy。

### 事件

与大家通常认为的*流*不同，Pipy 中的流是由**事件**组成，而非字节。事件的类型有四种：
