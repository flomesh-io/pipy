...
thread_local static const pjs::ConstStr s_connection_refused(
    "Connection Refused");
...
thread_local static const pjs::ConstStr s_application_octet_stream(
...
auto ResponseHead::error_to_status(StreamEnd::Error err, int &status)
    -> pjs::Str * {
  switch (err) {
...
Agent::Agent(pjs::Str *host, pjs::Object *options)
    : m_module(new Module), m_host(host) {
  ...
  m_pipeline_layout->append(new http::Mux(nullptr, options))
      ->add_sub_pipeline(pl_connect);
}
...
auto Agent::request(pjs::Str *method, pjs::Str *path, pjs::Object *headers,
                    Data *body) -> pjs::Promise * {
...
Directory::Options::Options(pjs::Object *options) {
  Value(options, "fs").get(fs).check_nullable();
  Value(options, "tarball").get(tarball).check_nullable();
  Value(options, "index").get(index).get(index_list).check_nullable();
  ...
  Value(options, "defaultContentType")
      ...
      .check_nullable();
  Value(options, "compression").get(compression_f).check_nullable();
}
...
Directory::Directory(const std::string &path) : Directory(path, Options()) {}
...
Directory::Directory(const std::string &path, const Options &options)
  ...
  if (auto a = options.index_list.get()) {
    a->iterate_all([this](pjs::Value &v, int) {
      auto s = v.to_string();
...
Directory::~Directory() { delete m_loader; }
...
auto Directory::serve(pjs::Context &ctx, Message *request) -> Message * {
  ...
  auto path =
      head->path ? utils::path_normalize(head->path->str()) : std::string();
  auto n = path.find('?');
  ...
  if (i == m_cache.end()) {
    ...
    if (!f.content_type) {
      ...
      f.content_type = i == m_content_types.end() ? m_default_content_type.get()
                                                  : i->second.get();
    }
...
void Directory::set_content_types(pjs::Object *obj) {
  ...
  if (obj) {
    obj->iterate_all([this](pjs::Str *k, pjs::Value v) {
      auto s = v.to_string();
...
auto Directory::get_encoded_response(pjs::Context &ctx, File &file,
                                     RequestHead *request) -> Message * {
  bool has_gz = false;
  bool has_br = false;
  ...
  if (accept_encoding.is_string()) {
    ...
    for (size_t i = 0; i < s.length(); i++) {
      ...
      if (i < s.length()) {
        auto n = 0;
        while (std::isalpha(s[i + n])) n++;
        if (n == 4 && !strncasecmp(&s[i], "gzip", n))
          has_gz = true;
        else if (n == 2 && !strncasecmp(&s[i], "br", n))
          has_br = true;
        i += n;
...
bool Directory::FileSystemLoader::load_file(const std::string &path,
                                            Data &data) {
...
void File::load(const std::string &filename,
  ...
  auto ct = (i == s_default_content_types.end() ? "application/octet-stream"
                                                : i->second);
...
auto File::to_message(pjs::Str *accept_encoding) -> pipy::Message * {
  ...
  if (has_br && m_data_br) {
    if (!m_message_br) {
      ...
      auto headers = Object::make();
      head->headers = headers;
      headers->set(pjs::EnumDef<StringConstants>::name(CONTENT_TYPE),
                   m_content_type.get());
      headers->set(pjs::EnumDef<StringConstants>::name(CONTENT_ENCODING),
...
bool File::decompress() {
  ...
  if (m_data_gz || m_data_br) {
    m_data = Data::make();
    auto func = [this](Data &data) { m_data->push(std::move(data)); };
    decomp =
        m_data_gz ? Decompressor::inflate(func) : Decompressor::brotli(func);
...
template <>
void EnumDef<StringConstants>::init() {
...
template <>
void ClassDef<MessageHead>::init() {
  field<pjs::Ref<pjs::Str>>("protocol",
                            [](MessageHead *obj) { return &obj->protocol; });
  field<pjs::Ref<pjs::Object>>("headers",
                               [](MessageHead *obj) { return &obj->headers; });
}

template <>
void ClassDef<MessageTail>::init() {
  field<pjs::Ref<pjs::Object>>("headers",
                               [](MessageTail *obj) { return &obj->headers; });
  field<int>("headSize", [](MessageTail *obj) { return &obj->headSize; });
  field<int>("bodySize", [](MessageTail *obj) { return &obj->bodySize; });
}

template <>
void ClassDef<RequestHead>::init() {
  super<MessageHead>();
  ctor();
  field<pjs::Ref<pjs::Str>>("method",
                            [](RequestHead *obj) { return &obj->method; });
  field<pjs::Ref<pjs::Str>>("scheme",
                            [](RequestHead *obj) { return &obj->scheme; });
  field<pjs::Ref<pjs::Str>>("authority",
                            [](RequestHead *obj) { return &obj->authority; });
  field<pjs::Ref<pjs::Str>>("path",
                            [](RequestHead *obj) { return &obj->path; });
}

template <>
void ClassDef<ResponseHead>::init() {
  super<MessageHead>();
  ctor();
  field<int>("status", [](ResponseHead *obj) { return &obj->status; });
  field<pjs::Ref<pjs::Str>>("statusText",
                            [](ResponseHead *obj) { return &obj->statusText; });
}

template <>
void ClassDef<Agent>::init() {
  ...
  method("request", [](Context &ctx, Object *obj, Value &ret) {
    ...
    if (ctx.try_arguments(1, &req) && req) {
      ...
      if (ctx.is_undefined(3)) {
        ...
        ret.set(
            static_cast<Agent *>(obj)->request(method, path, headers, body));
      } else {
...
template <>
void ClassDef<Constructor<Agent>>::init() {
  super<Function>();
  ctor();
}

template <>
void ClassDef<Directory>::init() {
  ctor([](Context &ctx) -> Object * {
    pjs::Str *path;
    pjs::Object *options = nullptr;
...
template <>
void ClassDef<Constructor<Directory>>::init() {
  super<Function>();
  ctor();
...
template <>
void ClassDef<Constructor<File>>::init() {
  super<Function>();
  ctor();
