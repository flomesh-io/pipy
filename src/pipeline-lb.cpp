...
std::map<std::string, PipelineAsyncWrapper::BalancerList> PipelineAsyncWrapper::m_registry;
std::mutex PipelineAsyncWrapper::m_registry_mutex;

void PipelineAsyncWrapper::update_pipeline_layouts(const std::set<PipelineLayout*> pipeline_layouts) {
  std::lock_guard<std::mutex> lock(m_registry_mutex);
  std::map<std::pair<std::string, std::string>, PipelineLayout*> new_pls;
  for (auto pl : pipeline_layouts) {
    new_pls[std::make_pair(
      static_cast<Module*>(pl->module())->filename()->str(),
      pl->name()->str()
    )] = pl;
  }
  auto net = &Net::current();
  for (auto &m : m_registry) {
    for (auto &b : m.second.balancers) {
      auto i = new_pls.find(std::make_pair(m.first, b.first));
      if (i == new_pls.end()) {
        auto &balancer = b.second;
        Target* back = nullptr;
        for (auto t = b.second.targets; t; t = t->next) {
          if (t->net == net) {
            auto n = t->next;
            delete t;
            if (balancer.current == t) {
              balancer.current = n;
            }
            if (back) {
              back->next = n;
...
void PipelineAsyncWrapper::add_pipeline_layout(const std::string &module, const std::string &name, PipelineLayout *layout) {
  std::lock_guard<std::mutex> lock(m_registry_mutex);
  auto *t = new Target;
  auto &b = m_registry[module].balancers[name];
  t->net = &Net::current();
  t->next = b.targets;
  b.targets = t;
}

void PipelineAsyncWrapper::remove_pipeline_layout(const std::string &module, const std::string &name) {
  std::lock_guard<std::mutex> lock(m_registry_mutex);
  auto net = &Net::current();
  auto balancer = find_balancer(module, name);
  if (!balancer) return;
  Target *b = nullptr;
  for (auto t = balancer->targets; t; t = t->next) {
    if (t->net == net) {
      auto n = t->next;
      delete t;
      if (balancer->current == t) {
...
auto PipelineAsyncWrapper::find_balancer(const std::string &module, const std::string &name) -> Balancer* {
  auto i = m_registry.find(module);
  if (i == m_registry.end()) return nullptr;
  auto j = i->second.balancers.find(name);
  if (j == i->second.balancers.end()) return nullptr;
  return &j->second;
}

PipelineAsyncWrapper::PipelineAsyncWrapper(const std::string &module, const std::string &name, EventTarget::Input *output)
  : m_output_net(&Net::current())
  , m_output(output)
{
  std::lock_guard<std::mutex> lock(m_registry_mutex);
  if (auto balancer = find_balancer(module, name)) {
    auto t = balancer->current;
    if (!t) t = balancer->targets;
    if (!t) return;
    balancer->current = t->next;
    m_pipeline_layout = t->layout;
    m_input_net = t->net;
