...
thread_local static const pjs::ConstStr s_transfer_encoding(
    "transfer-encoding");
...
auto Frame::decode_window_update(int &increment) const -> ErrorCode {
  ...
  increment = (((buf[0] & 0x7f) << 24) | ((buf[1] & 0xff) << 16) |
...
FrameDecoder::FrameDecoder() : m_payload(Data::make()) {
  Deframer::reset(STATE_HEADER);
  Deframer::read(sizeof(m_header), m_header);
}

void FrameDecoder::deframe(Data *data) { Deframer::deframe(*data); }

auto FrameDecoder::on_state(int state, int c) -> int {
  switch (state) {
    case STATE_HEADER: {
      const uint8_t *buf = m_header;
      auto size = ((uint32_t(buf[0]) << 16) | (uint32_t(buf[1]) << 8) |
                   (uint32_t(buf[2]) << 0));
      m_frame.type = buf[3];
      m_frame.flags = buf[4];
      m_frame.stream_id = ((uint32_t(buf[5]) << 24) | (uint32_t(buf[6]) << 16) |
                           (uint32_t(buf[7]) << 8) | (uint32_t(buf[8]) << 0)) &
                          0x7fffffff;
      if (size > m_max_frame_size) {
        on_deframe_error(FRAME_SIZE_ERROR);
        return -1;
      } else if (size > 0) {
        if ((m_frame.type == Frame::RST_STREAM && size != 4) ||
...
void FrameEncoder::header(uint8_t *buf, int id, uint8_t type, uint8_t flags,
                          size_t size) {
  buf[0] = 0xff & (size >> 16);
  buf[1] = 0xff & (size >> 8);
  buf[2] = 0xff & (size >> 0);
...
thread_local const HeaderDecoder::StaticTable HeaderDecoder::s_static_table;
const HeaderDecoder::HuffmanTree HeaderDecoder::s_huffman_tree;

HeaderDecoder::HeaderDecoder(const Settings &settings) : m_settings(settings) {}
...
auto HeaderDecoder::end(pjs::Ref<http::MessageHead> &head) -> ErrorCode {
  ...
  if (m_state != INDEX_PREFIX)
    return COMPRESSION_ERROR;  // incomplete header block
  if ((m_entry_prefix & 0xe0) == 0x20)
    return COMPRESSION_ERROR;  // ended with a table size change
  if (!m_is_response && !m_is_trailer) {
    auto req = head->as<http::RequestHead>();
    if (!req->method || !req->method->length() || !req->scheme ||
        !req->scheme->length() || !req->path || !req->path->length()) {
...
bool HeaderDecoder::add_field(pjs::Str *name, pjs::Str *value) {
  if (name->str()[0] == ':') {
    ...
    if (name == s_connection || name == s_keep_alive ||
        name == s_proxy_connection || name == s_transfer_encoding ||
        name == s_upgrade) {
      error(PROTOCOL_ERROR);
      return false;
...
void HeaderEncoder::encode(bool is_response, bool is_tail, pjs::Object *head,
                           Data &data) {
  ...
  if (headers.is_object()) {
    if (auto obj = headers.o()) {
      obj->iterate_all([&](pjs::Str *k, pjs::Value &v) {
...
void HeaderEncoder::encode_header_field(Data::Builder &db, pjs::Str *k,
                                        pjs::Str *v) {
...
void HeaderEncoder::encode_int(Data::Builder &db, uint8_t prefix,
                               int prefix_len, uint32_t n) {
...
void Endpoint::shutdown() { m_has_shutdown = true; }
...
void Endpoint::on_deframe(Frame &frm) {
  ...
  if (m_header_decoder.started() && frm.type != Frame::CONTINUATION) {
    ...
    if (frm.type == Frame::SETTINGS || frm.type == Frame::PING ||
      ...
      if (!stream) {
        if (id <= m_last_received_stream_id) {
          if (frm.type == Frame::PRIORITY || frm.type == Frame::RST_STREAM ||
      ...
      case Frame::SETTINGS:
        if (frm.is_ACK()) {
          ...
          if (len % 6) {
            ...
            if (err == NO_ERROR) {
              bool ok = true;
              if (m_peer_settings.initial_window_size !=
                  old_initial_window_size) {
                auto delta = m_peer_settings.initial_window_size -
                             old_initial_window_size;
                ok = for_each_stream([=](StreamBase *s) {
        ...
        if (err == NO_ERROR) {
          if (!inc) {
            ...
            if (n > 0x7fffffff) {
              ...
              m_send_window = n;
              for_each_pending_stream([this](StreamBase *s) {
...
void Endpoint::on_deframe_error(ErrorCode err) { connection_error(err); }
...
bool Endpoint::for_each_pending_stream(
    const std::function<bool(StreamBase *)> &cb) {
...
void Endpoint::end(StreamEnd *eos) {
  ...
  for_each_pending_stream([](StreamBase *s) {
    s->set_pending(false);
    return true;
  });

  if (eos) {
    for_each_stream([=](StreamBase *s) {
...
void Endpoint::clear() {
  for_each_stream([this](StreamBase *s) {
...
void Endpoint::init_metrics() {
  if (!s_metrics_initialized) {
    ...
    stats::Gauge::make(
        pjs::Str::make("pipy_http2_stream_count"), label_names,
        [=](stats::Gauge *gauge) {
...
// Endpoint::StreamBase
//
// For server-side: on_frame() -> event() ---(I)--> pipeline ---(O)-->
// on_event() -> frame() For client-side: on_event() -> frame() ---(O)-->
// pipeline ---(I)--> on_frame() -> event()
//
// A StreamBase is recycled when both its input and output ended.
//   - Input is ended passively as receiving a RST_STREAM
//   - Output is ended actively when a server shuts down or a client aborts a
//   stream
//
// Which end is the input or output depends on what side the endpoint is:
//   - For server-side, the input/output are the same ends as the pipeline for
//   the stream
//   - For client-size, the input/output are opposite to the ends of the
//   pipeline for the stream
//

static const int MAX_HEADER_FRAME_SIZE = 1024;

Endpoint::StreamBase::StreamBase(Endpoint *endpoint, int id,
                                 bool is_server_side)
    : m_endpoint(endpoint),
      m_id(id),
      m_is_server_side(is_server_side),
      ...
      if (!m_is_message_ended) {
        if (auto *end = evt->as<MessageEnd>()) {
          ...
          if (auto tail = end->tail()) {
            m_header_encoder.encode(m_is_server_side, true, tail,
                                    m_tail_buffer);
          }
        }
        if (m_state == OPEN) {
...
void Endpoint::StreamBase::on_frame(Frame &frm) {
  switch (frm.type) {
    ...
    case Frame::HEADERS: {
      ...
      } else if (m_state == IDLE || m_state == RESERVED_REMOTE ||
                 m_state == OPEN || m_state == HALF_CLOSED_LOCAL) {
        if (frm.is_PADDED() && !parse_padding(frm)) break;
...
bool Endpoint::StreamBase::parse_priority(Frame &frm) {
  ...
  auto dependency =
      0x7fffffff & (((uint32_t)buf[0] << 24) | ((uint32_t)buf[1] << 16) |
...
bool Endpoint::StreamBase::deduct_recv(int size) {
  ...
  if (m_is_clearing || connection_recv_window <= m_endpoint->m_recv_window_low)
    flush();
  return true;
}
...
void Endpoint::StreamBase::pump() {
  bool is_empty_end =
      (m_end_stream_send && m_send_buffer.empty() && m_tail_buffer.empty());
...
Server::~Server() { delete m_initial_stream; }
...
void Server::init() {
  if (m_initial_stream) {
    if (auto msg = m_initial_stream->initial_request()) {
      ...
      pjs::Ref<http::RequestHead> head =
          pjs::coerce<http::RequestHead>(msg->head());
...
Server::Stream::Stream(Server *server, int id) : StreamBase(server, id, true) {
...
// Request (output)
void Client::Stream::on_event(Event *evt) { StreamBase::encoder_input(evt); }
  ...
  switch (evt->type()) {
    ...
    case Event::Type::StreamEnd:
      if (!m_has_message_started) {
        ...
        auto status_code = 0;
        auto status_text =
            http::ResponseHead::error_to_status(eos->error_code(), status_code);
...
// Close endpoint
void Client::Stream::end() { end_input(); }
