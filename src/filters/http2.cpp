...
void FrameEncoder::frame(Frame &frm, EventTarget::Input *out) {
  ...
  out->input(data);
  if (frm.type == Frame::DATA && (frm.flags & Frame::BIT_END_STREAM)) {
...
void HeaderDecoder::add_field(pjs::Str *name, pjs::Str *value) {
  if (name == s_colon_method) {
    if (auto req = m_head->as<http::RequestHead>()) {
      req->method(value);
    }
    return;
    ...
    return;
  } else if (name == s_status) {
    if (auto res = m_head->as<http::ResponseHead>()) {
...
void HeaderEncoder::encode(bool is_response, pjs::Object *head, Data &data) {
  if (is_response) {
    ...
    if (status.is_number()) {
      auto s = pjs::Str::make(status.n());
      encode_header_field(data, s_colon_status, s);
      s->release();
    } else {
    ...
    pjs::Value scheme, method, path;
    if (head) {
      head->get(s_protocol, scheme);
    ...
    if (path.is_string()) {
      ...
      encode_header_field(data, s_colon_method, s_root_path);
    }
  }
...
void StreamBase::on_frame(Frame &frm) {
  switch (frm.type) {
    ...
    case Frame::HEADERS: {
      ...
      if (m_state == IDLE || m_state == RESERVED_REMOTE) {
        if (frm.is_PADDED() && !parse_padding(frm)) break;
...
void StreamBase::parse_headers(Frame &frm) {
  ...
  if (frm.is_END_HEADERS()) {
    m_state = (m_state == IDLE ? OPEN : HALF_CLOSED_LOCAL);
    event(MessageStart::make(m_header_decoder.head()));
    if (m_end_stream) {
...
void StreamBase::stream_error(ErrorCode err) {
  uint8_t buf[4];
  buf[0] = 0xff & (err >> 24);
  buf[1] = 0xff & (err >> 16);
  buf[2] = 0xff & (err >>  8);
  ...
  frm.flags = 0;
  s_dp.push(&frm.payload, buf, sizeof(buf));
  frame(frm);
}

void StreamBase::connection_error(ErrorCode err) {
}
