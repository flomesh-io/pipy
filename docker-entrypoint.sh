...
if [[ "$1" == "pipy" ]]; then
  if [[ "$2" == "docker-start" ]]; then
    ...
    if [ "${PIPY_CONFIG_FILE}x" == 'x' ]; then
      PIPY_CONFIG_FILE=/etc/pipy/test/001-echo/pipy.cfg
    fi
    if [ "$(id -u)" != "0" ]; then
      for i in $(seq 1 1 $PIPY_SPAWN); do
        exec /usr/local/bin/pipy ${PIPY_CONFIG_FILE} --watch-config-file --reuse-port &
      done
      exec /usr/local/bin/pipy ${PIPY_CONFIG_FILE} --watch-config-file --reuse-port
    else
      for i in $(seq 1 1 $PIPY_SPAWN); do
        exec su-exec pipy /usr/local/bin/pipy ${PIPY_CONFIG_FILE} --watch-config-file --reuse-port &
      done
      exec su-exec pipy /usr/local/bin/pipy ${PIPY_CONFIG_FILE} --watch-config-file --reuse-port 
    fi
  fi
